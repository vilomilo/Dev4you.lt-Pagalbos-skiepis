...
class LicenseBoxExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = '3A1E2754';
		$this->api_url = 'https://license.dev4you.lt/';
		$this->api_key = 'FC889241B6B134160153';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["35602cf9ca80b24"])){
				$_SESSION["35602cf9ca80b24"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["35602cf9ca80b24"])){
				$get_data = $this->call_api(
					'POST',
